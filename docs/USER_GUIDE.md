...
7. [工具安装示例](#7-工具安装示例)
8. [配置管理](#8-配置管理)
9. [飞书 MCP 私有化部署](#9-飞书-mcp-私有化部署)
10. [卸载操作](#10-卸载操作)
11. [国内镜像加速](#11-国内镜像加速)
12. [常见问题 (FAQ)](#12-常见问题-faq)
13. [配置文件速查表](#13-配置文件速查表)
...
配置飞书/Lark MCP 私有化部署，将飞书文档能力接入 AI 编程工具。

```bash
curl -fsSL https://tinyurl.com/25n5uezk | bash -s -- lark-mcp
```

进入 TUI 表单填写凭证，然后选择目标 AI 工具（Claude / Cursor / Windsurf / VS Code / Codex）。详见 [9. 飞书 MCP 私有化部署](#9-飞书-mcp-私有化部署)。
...
为 AI 编程工具接入私有化部署的飞书/Lark 文档服务。一次填写凭证，同时配置到多个 AI 工具。

### 9.1 前置条件

- 飞书开放平台创建应用，获取 **App ID** 和 **App Secret**
- 应用已开启「文档」相关权限（如 `docx.v1.document.rawContent`）
- 已知私有化部署地址（如 `https://open.example.com`）
...
行内编辑模式支持光标移动（`←` `→`）、`Home` / `End` 跳转、`Backspace` 删除。App Secret 全程脱敏显示。
...
| 字段 | 说明 | 默认值 |
|------|------|--------|
| App ID | 飞书开放平台应用 ID | (必填) |
| App Secret | 飞书开放平台应用密钥 | (必填) |
| 部署地址 | 私有化飞书 API 地址 | `https://open.example.com` |
| API 工具列表 | 开放的 API 能力，逗号分隔 | `docx.v1.document.rawContent` |
| 认证模式 | token 获取方式 | `user_access_token` |

#### Step 2 — 选择目标 AI 工具（多选）
...
| AI 工具 | 配置路径 | 格式 | 检测方式 |
|---------|---------|------|---------|
| Claude Code | `~/.claude.json` | `claude mcp add` CLI | `which claude` |
| Cursor | `~/.cursor/mcp.json` | JSON `mcpServers` | `~/.cursor/` 目录 |
| Windsurf | `~/.codeium/windsurf/mcp_config.json` | JSON `mcpServers` | `~/.codeium/windsurf/` 目录 |
| VS Code Copilot | `~/.vscode/mcp.json` | JSON `mcpServers` | `which code` |
| Codex (OpenAI) | `~/.codex/config.toml` | TOML `[mcp_servers]` | `which codex` |
...
[ OK ] VS Code Copilot: ~/.vscode/mcp.json
```

如果认证模式为 `user_access_token`，随后提示进行 OAuth 登录（浏览器自动打开授权页面）。

### 9.4 各工具使用示例

配置完成后，重启对应 AI 工具即可使用。
...
claude
# 在对话中：
> 帮我总结这个飞书文档的内容 https://your-domain.com/docx/doxrXXXXX
> 读取飞书文档并提取其中的风控指标定义 https://your-domain.com/docx/doxrYYYYY
```

Claude Code 会自动调用 `lark-mcp` 工具获取文档纯文本内容，然后进行分析和总结。

#### Cursor

在 Cursor 编辑器的 AI Chat 面板（`Cmd+L` / `Ctrl+L`）中：

```
读取飞书文档 https://your-domain.com/docx/doxrXXXXX 并总结要点
```

验证配置：`Cursor → Settings → MCP` 应显示 `lark-mcp` 服务器为绿色 ✓ 状态。
...
验证配置：`Windsurf → Settings → MCP Servers` 应显示 `lark-mcp`。

#### VS Code (GitHub Copilot)

在 VS Code 中打开 Copilot Chat（`Ctrl+Shift+I`）：

```
#lark-mcp 总结飞书文档 https://your-domain.com/docx/doxrXXXXX
```

验证配置：命令面板 `Ctrl+Shift+P` → `MCP: List Servers` 应显示 `lark-mcp`。
...
| 模式 | 说明 | 适用场景 |
|------|------|---------|
| `user_access_token` | 以用户身份访问，可读取个人有权限的所有文档 | 个人使用，权限跟随用户 |
| `tenant_access_token` | 以应用身份访问，只能读取应用被显式授权的文档 | 团队共享，无需个人登录 |

#### Token 过期处理

`user_access_token` 有有效期，过期后工具会报错 `token is invalid or expired`。重新登录：
...
浏览器打开后完成授权即可。所有已配置的 AI 工具共享同一个 token，无需逐个重新配置。

### 9.6 故障排查

| 问题 | 原因 | 解决方案 |
|------|------|---------|
| `token is invalid or expired` | access_token 过期 | 重新执行 `npx lark-mcp login` |
| 工具列表中看不到 lark-mcp | 配置未生效 | 重启 AI 工具 |
| `ECONNREFUSED` 连接失败 | 部署地址不可达 | 检查 Base URL 和网络连通性 |
| `permission denied` | 文档无访问权限 | 检查应用权限或文档共享设置 |
| Codex 中 MCP 不工作 | TOML 格式问题 | 检查 `~/.codex/config.toml` 语法 |
